	...
	for {
		...
		if strings.Contains(obj.Name, bslPrefix+backupObject+"/") {
			...
			if err = bucket.Object(obj.Name).Delete(ctx); err != nil {
				return errors.Wrapf(err, "%s", fmt.Sprintf("Fail to delete object %s in bucket %s", obj.Name, bslBucket))
			}
		}
	}
}
...
func (s GCSStorage) IsSnapshotExisted(cloudCredentialsFile, bslConfig, backupObject string, snapshotCheck test.SnapshotCheckPoint) error {
	...
	if err != nil {
		return errors.Wrapf(err, "%s", fmt.Sprintf("Failed reading gcloud credential file %s", cloudCredentialsFile))
	}

	creds, err := google.CredentialsFromJSON(ctx, data)
	if err != nil {
		return errors.Wrapf(err, "%s", fmt.Sprintf("Failed getting credentials from JSON data %s", string(data)))
	}
