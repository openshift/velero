...
func (e *ExcludeResources) Verify() error {
	for nsNum := 0; nsNum < e.NamespacesTotal; nsNum++ {
		...
		if err != nil {
			...
			return errors.Errorf("%s", fmt.Sprintf("Should no secrets found  %s in namespace: %q", secretsList.Items[0].Name, namespace))
		}
		...
		if err != nil {
			...
			return errors.Errorf("%s", fmt.Sprintf("Should have configmap found in namespace: %q", namespace))
		}
	}
	return nil
}
